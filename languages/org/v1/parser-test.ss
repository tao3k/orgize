    ...
    (test-case "Org owns sections, drawers, and greater blocks"
      (let* ((structure org-v1-line-structure)
        ...
        (check (list-line-item-node (line-structure-list structure))
               => 'OrgListItem)
        (check (map key-line-node key-lines)
               => '(OrgBabelCall OrgKeyword OrgPlanning OrgClock))
        (check (map key-line-prefix key-lines) => '("#+" "#+" "" ""))
        (check (key-line-keys (car key-lines)) => '("CALL"))
        (check (key-line-keys (caddr key-lines))
               => '("SCHEDULED" "DEADLINE" "CLOSED"))
        (check (key-line-after-heading (caddr key-lines)) => #t)
        (check (key-line-repeated (caddr key-lines)) => #t)
        (check (key-line-keys (cadddr key-lines)) => '("CLOCK"))
        (check (key-line-after-heading (cadddr key-lines)) => #f)))))
