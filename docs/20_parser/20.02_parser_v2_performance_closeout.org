...
and the current dense-path evidence collected on 2026-05-12, with the
dense-agenda and include/datetree rows refreshed on 2026-05-15.
...
| `Document::include-datetree-agenda-extras/dense` | `include-expansion-plan.org`, `datetree-entries.org`, `agenda-inactive-diary.org` | Include expansion planning, datetree recognition, inactive timestamp projection, and diary timestamp agenda rows. |
...
direnv exec . cargo bench --bench parse include-datetree -- --sample-size 10 --warm-up-time 1 --measurement-time 2
#+end_src
...
| `Document::agenda_entries/dense-agenda/many-agenda-planning-timestamps.org` | 1.5405 ms | 1.4914-1.6307 ms | 26.863 MiB/s |
| `Document::include-datetree-agenda-extras/dense/include-expansion-plan.org` | 97.327 us | 89.396-111.76 us | 271.38 MiB/s |
| `Document::include-datetree-agenda-extras/dense/datetree-entries.org` | 82.463 us | 78.602-90.595 us | 320.30 MiB/s |
| `Document::include-datetree-agenda-extras/dense/agenda-inactive-diary.org` | 161.02 us | 150.65-176.77 us | 164.03 MiB/s |
...
direnv exec . cargo bench --bench parse include-datetree -- --test
direnv exec . cargo bench --bench parse "dense-|macro_expansions/dense" -- --sample-size 10 --warm-up-time 1 --measurement-time 2
...
The ASP Rust API-backed policy test must report zero findings. The boundary grep must return no
