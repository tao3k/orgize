* 20.01 Parser v2 Release Readiness
...
[[file:20.02_parser_v2_performance_closeout.org][20.02 Parser v2 Performance Closeout]].
...
- Agent planning snapshots are available through
  `Document::agent_planning_snapshot(&AgentPlanningQuery)`. They derive compact
  cards from existing agenda rows and source annotations without defining new
  Org source syntax.
...
The ASP Rust API-backed policy tests should report zero findings. The local `.data/`
...
- Do not downgrade or disable ASP Rust policy to land this lane.
- Keep root and WASM build scripts thin by mounting only
  `asp-rust-build-support`; run full ASP Rust source policy from cargo-test
  gates at each repository boundary.
