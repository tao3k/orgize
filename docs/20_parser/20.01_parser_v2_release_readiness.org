...
- Keep the Rust project harness mounted from root `build.rs` and the
  `src/lib.rs` cargo-test gate for the parser workspace. The `wasm/build.rs`
  gate now lives in the `tao3k/orgize-wasm` submodule so browser-package policy
  is enforced at its own repository boundary.
