...
- [ ] Extend special property projection and match evaluation with official
      special properties and timestamp-aware operands.
- [ ] Preserve parsed `LinkSearch` suffixes as typed metadata and
      document-local resolution hints.
- [ ] Document the Wendao adapter contract for consuming Orgize projections in
      search indexes.
- [ ] Feed richer archive, lifecycle, attachment, and habit authority into
      `AgentMemorySnapshot`.
...
- [ ] Model `#+RESULTS:` blocks/keywords as first-class semantic records linked
      to the preceding source or inline source block.
- [ ] Parse and normalize common Babel header args beyond the current raw
      `:key value` projection: `:results`, `:exports`, `:eval`, `:session`,
      `:var`, `:noweb`, `:tangle`, `:dir`, and `:cache`.
- [ ] Add a source-block side table for language, name, result, header args,
      coderefs, and tangle target metadata.
- [ ] Add lint rules for missing block names referenced by noweb or calls,
      unsafe/eval-sensitive headers, duplicate `#+NAME:`, and invalid tangle
      targets.
- [ ] Keep execution out of orgize; only parse, index, lint, format, and
      project metadata.
...
- [ ] Extend the same typed path metadata and optional diagnostics to ordinary
      `file:` links.
- [ ] Add formatter/path tests for file links and attachment-style links that
...
- [ ] Parse table formulas into typed references, assignments, and flags while
      preserving raw `#+TBLFM:` source.
- [ ] Add lint rules for malformed table formulas and formula references that
      point outside the table shape.
- [ ] Extend `orgize fmt` table alignment to preserve formula boundaries,
      hlines, cookies, and indentation across more official table examples.
- [ ] Model column view declarations (`COLUMNS`, `#+COLUMNS:`) as typed document
...
- [ ] Expand the typed `#+OPTIONS:` model beyond `H`, `-`, and `e` to cover the
      high-usage export toggles for TODOs, tags, timestamps, author/date/title,
      drawers, planning, priorities, broken links, and section numbering.
- [ ] Model `#+BIND:`, `#+SETUPFILE:`, `#+EXPORT_FILE_NAME:`, and backend
      `#+ATTR_*` settings as side-table metadata without executing Emacs Lisp.
- [ ] Add an explicit include expansion plan for `#+INCLUDE:` line/range/options
      semantics; keep expansion opt-in and filesystem-aware only in lint/export
      helper APIs.
- [ ] Keep publishing out of core parser execution, but parse publishing-related
...
- [ ] Project diary sexp timestamps as explicit unsupported-or-diary agenda
      rows with diagnostics instead of silently ignoring them.
- [ ] Add habit metadata projection for `STYLE: habit`, repeaters, scheduled
      state, and consistency graph inputs, without implementing Emacs agenda
      graph rendering.
- [ ] Recognize datetree shape for date-organized capture files and expose a
      helper that maps dates to heading paths.
- [ ] Add tests for inactive timestamp inclusion as an explicit agenda option,
