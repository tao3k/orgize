...
- [ ] Add raw memory record structs over headings, properties, timestamps,
      drawers, LOGBOOK-like content, links, and source spans.
- [ ] Add `MemoryQuery` and a conservative official-Org-first default profile.
- [ ] Add `AgentMemorySnapshot` compact cards that distinguish current, stale,
      closed, archived, conflicting, and background facts.
- [ ] Add fixtures proving stale or corrected facts do not re-enter current
      decisions merely because they remain searchable.
...
- [ ] Replace `Section::priority: Option<String>` with a typed priority domain
      carrying the normalized priority cookie and source value.
- [ ] Parse default priority semantics (`A`, `B`, `C`) into projection metadata
      while keeping raw priority text for compatibility.
- [ ] Parse common duration values from `EFFORT`, clock summaries, and
      property drawer values into typed duration metadata.
- [ ] Add a property side-table/index for fast lookup by inherited and local
      properties without reparsing drawers during agenda/lint projections.
- [ ] Add lint rules for malformed priority cookies, malformed duration values,
