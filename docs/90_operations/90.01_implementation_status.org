...
- [ ] Parse lifecycle/logbook drawers into semantic records for state changes,
      notes, refiles, reschedules, redeadlines, and clock history.
- [ ] Model archive semantics beyond the `ARCHIVE` tag: `ARCHIVE` property,
      archive location keywords, and archived subtree intent.
- [ ] Add memory projection over standard Org constructs: heading, TODO state,
