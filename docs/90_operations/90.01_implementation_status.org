...
- [ ] Add source-grounded section/index support records for Wendao Search
...
- [ ] Model attachment directories from `ID`, `DIR`, `ATTACH_DIR`,
      `ATTACH_DIR_INHERIT`, and attachment-related properties without touching
      the filesystem by default.
- [ ] Resolve attachment links and file links into typed semantic link metadata
      with path kind, locality, and optional existence diagnostics for lint.
- [ ] Add lint checks for missing local file/attachment targets when linting a
      real file path, while keeping parser output filesystem-independent.
