...
/// Run a document command with the same arguments accepted by the `orgize org` or
/// `orgize md` document facade after the language id.
...
/// Run a document command with explicit caller-supplied project-walk
/// configuration.
...
/// Run an Orgize CLI command through its `orgize org` facade.
