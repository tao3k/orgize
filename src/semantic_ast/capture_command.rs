...
const CAPTURE_PLAN_USAGE: &str = "Usage: orgize org capture --title TITLE [--contract CONTRACT_ID --org-contract-registry PATH.org] [--kind task|note|decision|idea|evidence|preference|correction|memory-candidate|article-note] [--body TEXT] [--quote TEXT] [--target inbox|datetree|outline|current-section] [--target-file PATH] [--outline A/B] [--date YYYY-MM-DD] [--insert append|prepend|first-child|last-child] [--tag TAG] [--property KEY=VALUE] [--source-url URL] [--source-label LABEL] [--actor ACTOR] [--memory-policy none|candidate|background|decision|transient|supersedes] [--no-confirm]";
